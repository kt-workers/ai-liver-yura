...
@media(max-width:900px){
  ...
  .toolbar select, .run-state { grid-column:1/-1; }
...
<section class="card">
  ...
  <details><summary>Constraints / Character</summary>
    ...
    <label><input id="includePrompts" type="checkbox" style="width:auto"> Promptも結果に含める</label>
  </details>
...
async function loadPresets() {
  ...
  $('preset').innerHTML = Object.entries(presets).map(([key, value]) => `<option value="${key}">${value.label}</option>`).join('');
...
$('run').onclick = async () => {
  ...
  $('status').textContent = 'Character / Validator pipelineを実行しています…';
  $('run').disabled = true;
  $('load').disabled = true;
  try {
    const r = await fetch('/api/character-response', {
      method: 'POST',
