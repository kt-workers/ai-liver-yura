...
class GitHubClient:
    ...
    def get_pull(self, pr_number: int) -> dict[str, object]:
        ...
        if not isinstance(result, dict):
            raise GitHubApiError("invalid pull request response")
        return result
    ...
    def get_issue(self, issue_number: int) -> dict[str, object]:
        ...
        if not isinstance(result, dict):
            raise GitHubApiError("invalid issue response")
        return result
    ...
    def get_content(self, path: str, ref: str) -> str:
        ...
        if not isinstance(result, dict) or result.get("encoding") != "base64":
            raise GitHubApiError(f"invalid contents response for {path}@{ref}")
    ...
    def list_reviews(self, pr_number: int) -> list[dict[str, object]]:
        result = self._json(
            "GET", f"/repos/{self.repository}/pulls/{pr_number}/reviews?per_page=100"
        )
