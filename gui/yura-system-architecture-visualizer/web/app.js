...
function fitToView(){const bounds=viewport.getBBox();if(!bounds.width||!bounds.height)return;const rect=svg.getBoundingClientRect(),pad=55;scale=Math.min((rect.width-pad*2)/bounds.width,(rect.height-pad*2)/bounds.height,1.2);translate.x=(rect.width-bounds.width*scale)/2-bounds.x*scale;translate.y=(rect.height-bounds.height*scale)/2-bounds.y*scale;applyTransform();}
async function loadGraph(){loading.hidden=false;try{const response=await fetch('/api/graph',{cache:'no-store'});if(!response.ok)throw new Error(`HTTP ${response.status}`);graphData=await response.json();populateFilters();render();document.getElementById('generatedAt').textContent=`解析: ${new Date(graphData.generated_at).toLocaleString()} · diagnostics ${graphData.diagnostics?.length||0}`;requestAnimationFrame(fitToView);}catch(error){document.getElementById('generatedAt').textContent=`読み込み失敗: ${error.message}`;}finally{loading.hidden=true;}}
searchInput.addEventListener('input',applySearchAndSelection);categoryFilter.addEventListener('change',()=>{render();requestAnimationFrame(fitToView);});document.getElementById('fitButton').addEventListener('click',fitToView);document.getElementById('reloadButton').addEventListener('click',loadGraph);document.getElementById('closeDetail').addEventListener('click',clearSelection);svg.addEventListener('click',clearSelection);
