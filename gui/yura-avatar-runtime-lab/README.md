...
- メインCanvasの描画済みフレームを `mobile-preview.js` が転写するため、表情・視線・ジェスチャーの表示差は発生しません。
...
1. スマートフォンまたはブラウザのモバイル表示で画面を開く。
2. Manual Probeまでスクロールする。
3. 上端のLive Previewが残ることを確認する。
4. 表情、ジェスチャー、視線の各ボタンを押し、Live Previewへ即時反映されることを確認する。
