"""HTTP client for the standalone Streaming Subsystem Admin API."""
...
class StreamingSubsystemApiError(RuntimeError):
    def __init__(
        self,
        code: str,
        message: str,
...
class StreamingSubsystemApiClient:
    def __init__(self, config: StreamingSubsystemAdminConfig) -> None:
        self.config = config
    ...
    def _request(self, method: str, path: str, json: dict[str, Any] | None = None) -> Any:
        try:
            ...
            raise StreamingSubsystemApiError(
                "streaming_subsystem.unavailable",
                "Streaming Subsystemへ接続できません。",
                True,
                503,
            ) from error
        try:
            ...
            raise StreamingSubsystemApiError(
                "streaming_subsystem.invalid_response",
                "Streaming Subsystemから不正な応答を受信しました。",
                True,
                502,
            ) from error
        if response.is_error:
            ...
            raise StreamingSubsystemApiError(
                str(detail.get("code", "streaming_subsystem.request_failed")),
                str(detail.get("message", "Streaming Subsystem API request failed")),
    ...
    def manual_check_ui_event(self, _event: str, _details: dict[str, Any] | None = None) -> None:
        return None
