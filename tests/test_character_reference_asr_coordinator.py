...
@pytest.mark.asyncio
async def test_same_revision_is_not_transcribed_twice(tmp_path: Path) -> None:
    media = tmp_path / "reference.mov"
    media.write_bytes(b"media")
    backend = FakeBackend()
    store = InMemoryStore()
...
async def test_failure_is_recorded_and_requires_explicit_retry(tmp_path: Path) -> None:
    ...
    skipped = await coordinator.process(source, media)

    assert failed.outcome == AsrProcessingOutcome.FAILED
    assert skipped.outcome == AsrProcessingOutcome.SKIPPED_DUPLICATE
