...
async def test_semantic_lab_exports_new_pipeline_boundaries() -> None:
    ...
    observation = result["realization_observation"]
    assert isinstance(observation, dict)
    observations = observation["observations"]
    assert isinstance(observations, list)
    assert observations[0]["realization_id"] == "proposition:0:joy"
    assert observations[0]["observed_state"] == "absent"
    assert observations[0]["observed_certainty"] == "high"

    realization = result["realization_validation"]
    assert isinstance(realization, dict)
    assert realization["accepted"] is True
    assert realization["reason"] == "post_observation_semantic_contract_consistent"

    assert result["pipeline_boundaries"] == [
...
async def test_semantic_lab_exports_final_regeneration_attempt(monkeypatch: pytest.MonkeyPatch) -> None:
    async def _synthetic_analyze(
        ...
        return {
            ...
            "model_calls": [
                {
                    ...
                    "parsed_response": {"speech": "元気はあるよ。"},
                    "context_keys": ["semantic_boundary"],
                    "semantic_boundary": True,
                },
                {
                    "role": "character_realization_observer",
                    "parsed_response": {
                        "observations": [
                            {
                ...
                {
                    "role": "character_language_realizer",
                    "parsed_response": {"speech": "今はエネルギーが低めだよ。"},
                    "context_keys": ["semantic_boundary"],
                    "semantic_boundary": True,
                },
                {
            ...
            "final_response": {
                "speech": "今はエネルギーが低めだよ。",
                "linguistic_performance": {"phrasing": ["今はエネルギーが低めだよ。"]},
    ...
    assert result["character_utterance"] == {"speech": "今はエネルギーが低めだよ。"}
    observation = result["realization_observation"]
    assert isinstance(observation, dict)
    assert observation["observations"][0]["observed_state"] == "low"
    realization = result["realization_validation"]
    assert isinstance(realization, dict)
    assert realization["accepted"] is True
    assert realization["reason"] == "post_observation_semantic_contract_consistent"
...
@pytest.mark.asyncio
async def test_semantic_lab_records_sanitized_character_observer_and_validator_model_boundaries() -> None:
    service = CharacterSemanticResponseLabService(_settings())
    ...
    assert observer_boundary["role"] == "character_realization_observer"
    ...
    assert forbidden_keys.isdisjoint(observer_boundary["context_keys"])
