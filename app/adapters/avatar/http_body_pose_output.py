...
class BodyPosePayload(Protocol):
...
class HttpBodyPoseOutputConfig:
    """BodyPoseFrame受信モックまたはAvatar Runtimeへ接続する設定。"""
...
class HttpBodyPoseFrameOutput:
    """Coreが生成した最新Pose Frameだけを非同期でHTTP送信する。

    Queueは1件に制限し、送信待ち中に新しいFrameが来た場合は古いFrameを
    破棄する。描画モックやAvatar Runtimeの遅延でCore Body Tickを停止させない。
    """
    ...
    def __init__(
        ...
        *,
        ...
        self._queue: asyncio.Queue[BodyPosePayload] = asyncio.Queue(maxsize=1)
    ...
    async def publish_body_pose_frame(self, frame: BodyPosePayload) -> None:
        if self._closed:
            return
    ...
    async def _run(self) -> None:
        while True:
            ...
            try:
                ...
                raise
            except Exception as error:  # Network failure must not stop Body.
    ...
    async def _send(self, frame: BodyPosePayload) -> None:
        payload = {
