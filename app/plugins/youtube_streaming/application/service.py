...
class StreamingApplicationService:
    def __init__(
        self,
        runtime: StreamingRuntimeComponents,
        broker: ApplicationEventBroker | None = None,
        *,
        demo_mode: bool = False,
        manual_check_log: ManualCheckRecorder | None = None,
        repository_factory: StreamingRepositoryFactory,
    ) -> None:
        self.runtime = runtime
        self.broker = broker or ApplicationEventBroker()
        ...
        self._activity_gateway: CoreActivityGateway | None = None
    ...
    def configure_opening(self, activity_gateway: CoreActivityGateway) -> None:
        ...
        activity_gateway.configure_lifecycle_gate(self._lifecycle_gate)
        ...
        self._comment_ranking = CommentRankingUsecase(
            ...
            candidates=self._repositories.candidates(ranking_settings.max_pool_size),
            rankings=self._repositories.rankings(),
            selections=self._repositories.selections(),
            history=self._repositories.ranking_history(ranking_settings.history_size),
            settings=ranking_settings,
            publisher=publish,
        )
        self._comment_response = CommentResponseUsecase(
            gate=self._lifecycle_gate,
            activities=self._repositories.response_activities(),
            selections=self._comment_ranking,
            history=self._repositories.response_history(),
        ...
        self._comment_moderation = CommentModerationUsecase(
            gate=self._lifecycle_gate,
            repository=self._repositories.moderation(),
