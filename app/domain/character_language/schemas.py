...
def character_language_instructions() -> str:
    ...
    return "\n".join(
        (
            ...
            "  直近same-Plan表現です。How-to-say上のweak repetition-awareness referenceとしてだけ使ってください。",
            (
                "- prior_realizationsをFact source、会話履歴、追加propositionとして"
                "扱ってはいけません。"
            ),
            "- actual meaningはcurrent semantic_planだけから決めてください。",
            "- semantic preservation、自然さ、Characterらしさをrepetition avoidanceより優先してください。",
            "- equally naturalかつ意味安全な代替が明らかにある場合だけ、priorとの過度な",
            "  exact/near-exactな語彙、語順、rhythm、締め方への収束を避けても構いません。",
            "- priorと同じ表現を使う方が自然で意味安全なら、そのまま再使用して構いません。",
            "- priorとの差を作るために意味を追加・削除・弱化・強化してはいけません。",
            "- certaintyを弱める婉曲表現、不自然な同義語置換、過剰なCharacter演技で差分を作らないでください。",
            "- uniqueな文を生成すること自体を目標にしないでください。",
            "",
            "Constraints:",
