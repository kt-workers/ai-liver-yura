...
_LEGACY_STATE_TO_V2: dict[
    str, tuple[SemanticValueStatus, SemanticPolarity | None, SemanticDegree | None, SemanticSummaryMode]
] = {
...
class SemanticPropositionV2:
    ...
    def __post_init__(self) -> None:
        ...
        if self.concept is not None and not self.concept.strip():
...
class SemanticUtterancePlanV2:
    ...
    def __post_init__(self) -> None:
        ...
        for values in (
            ...
            if any(not item.strip() for item in values):
                raise ValueError(
                    "SemanticUtterancePlanV2の文字列配列に空文字は使用できません。"
                )
        ...
        if any(
            ...
            raise ValueError("discourse_contextは非空文字列のkey/valueだけを使用してください。")
    ...
    def from_context(cls, value: object) -> SemanticUtterancePlanV2 | None:
        ...
        if target_value is None:
            ...
            target_type = cls._required_string(target_value.get("type"))
            target_id = cls._required_string(target_value.get("id"))
        ...
        if None in (required_content, optional_content, forbidden_additions, reasons):
            return None
        ...
        if response_length not in _ALLOWED_LENGTH or self_disclosure not in _ALLOWED_DISCLOSURE:
            return None
        ...
        if not isinstance(interpersonal_value, Mapping):
            return None
        interpersonal_keys = {
        ...
        if not interpersonal_keys.issubset(interpersonal_value.keys()):
            return None
        interpersonal_values = {
            key: cls._required_string(interpersonal_value.get(key))
            for key in interpersonal_keys
        }
        if any(item is None for item in interpersonal_values.values()):
            return None
        interpersonal = InterpersonalContentContext(
            disclosure_permission=cast(str, interpersonal_values["disclosure_permission"]),
            boundary_sensitivity=cast(str, interpersonal_values["boundary_sensitivity"]),
            social_distance=cast(str, interpersonal_values["social_distance"]),
            current_tension=cast(str, interpersonal_values["current_tension"]),
        )

        discourse_value = value.get("discourse_context")
        if not isinstance(discourse_value, Mapping):
            return None
        discourse_context: dict[str, str] = {}
        for key, item in discourse_value.items():
            if (
                not isinstance(key, str)
                or not key.strip()
                ...
                return None
            discourse_context[key.strip()] = item.strip()
        ...
        try:
            return cls(
                ...
                target=target,
                propositions=tuple(propositions),
                required_content=cast(tuple[str, ...], required_content),
                optional_content=cast(tuple[str, ...], optional_content),
                forbidden_additions=cast(tuple[str, ...], forbidden_additions),
    ...
    @classmethod
    def _proposition_from_context(cls, value: object) -> SemanticPropositionV2 | None:
        if not isinstance(value, Mapping):
            return None
        required_keys = {
            "proposition_id",
        ...
        if not isinstance(raw_value, Mapping):
            return None
        if not {"status", "polarity", "degree", "certainty"}.issubset(raw_value.keys()):
            return None

        status = cls._required_string(raw_value.get("status"))
        certainty = cls._required_string(raw_value.get("certainty"))
        polarity = cls._optional_string(raw_value.get("polarity"))
        degree = cls._optional_string(raw_value.get("degree"))
        if (
            status not in _ALLOWED_STATUS
            or certainty not in _ALLOWED_CERTAINTY
            or (polarity is not None and polarity not in _ALLOWED_POLARITY)
            or (degree is not None and degree not in _ALLOWED_DEGREE)
        ):
            return None
