...
codecはDomain DTO外に置く。messageは64KiBを上限とし、重複key・未知field・不正数値・不正identityを拒否する。stdoutはprotocol専用、Adapterのstdout/stderrは人間向け出力先へ分離し、親で生の外部例外を診断に採用しない。
...
OSがkill後も回収完了を返さない場合はtyped cleanup failureとして失敗を表面化し、成功postconditionを主張しない。未回収processを追跡から消さない。
