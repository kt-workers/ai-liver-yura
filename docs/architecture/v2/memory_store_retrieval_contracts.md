...
semantic indexを使う検索には、Memory Domainの`FinalizableMemorySemanticIndex`を明示的に登録する。wrapperはrank 58のparticipantを持ち、upsertとrelated_scoresを同じ境界へ参加させる。raw indexを自動で安全とみなさず、未登録indexを使用する現在公開はPARTICIPANT_UNSUPPORTEDで拒否する。既存retrieveは変更しない。wrapper外からのindex変更は保証対象外。semantic queryは既存のindex全体のtop-Nをranking入力にするため、index更新はそのindexに依存する検索を失効させる。semantic queryを使わない検索にはindex tokenを付けず、index更新で失効させない。

policy tokenを先に取得し、検索集合、必要ならindexの順に読取境界を取得する。途中のpolicy更新は既存のpolicy freshness検査または最終Fenceのgeneration不一致で拒否する。degraded viewの内容は保持するが、最終採用可能なtokenを返さず、成功した現在公開として使用させない。

### Fence・資源・構成境界

ID 55 → 検索集合56 → policy57 → index58の全順序に従う。Fence自身ではDB読取・ranking・遅いI/Oを行わず、既存の非待機取得によるBUSY・generation mismatchで拒否する。検索readやmatching writeの遅さを、無関係なquery/IDへ新しい全体lockで伝播させない。SQLite等の既存storage制約を緩和したとは主張しない。
