...
Repository label `v2` をV2対象の識別子とする。

- IssueとPull Requestの両方へ個別付与
- Issue labelが関連PRへ自動継承されると考えない
- Project Auto-add workflowは `label:v2` を条件とする
- `v2` labelのないIssue / PRをV2と自動推測しない
- 既存Issue / PRを一括変更せず、対象scopeを確定してから変更する
...
**Issue本文に記載されたV2のAreaと一致するProject optionを使用する。**

旧Project #6のbroad Area taxonomyをV2 #7へ自動流用しない。
...
7. Project都合でIssue本文 / label / Assignees / Milestone等を勝手に変更しない
...
ただしProject #7管理基盤への切替 #367 が完了するまで、新規product implementation lineageの開始を一時保留する。
