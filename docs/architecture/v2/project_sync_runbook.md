# ゆらV2のProject同期手順と初期同期の履歴

## 現在の同期手順

現在の対象は`ktan514 / Project #7`「プロジェクトゆらv2」である。対象リポジトリはGitHubの現在状態から確認する。現在の製造順・依存関係は[現行製造計画](production_plan_current.md)と対象Issue、作業位置はIssue #450の最新Checkpoint、日程整理はIssue #425を参照する。

1. 認証と対象Projectを確認し、現在のフィールド・選択肢・対象項目の識別子を取得する。
2. 対象リポジトリとIssue番号を照合し、変更前の値を読む。以下の履歴にある分類名、日付、識別子を現在値として使わない。
3. 今回承認された変更項目と変更後の値を確定する。既にその値なら書き込まない。
4. 必要な項目だけを変更する。別途指示がない日程、担当、優先度、階層等を同時に変更しない。
5. 対象と変更対象外の値を再取得し、意図した変更だけが反映されたことを確認して記録する。

Project #6は変更しない。以下は初期同期時の対象・分類・操作の記録であり、そのまま現在のProjectへ再実行する手順ではない。

## 2026-08-13の初期同期手順（履歴）
