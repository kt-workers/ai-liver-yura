...
`system_architecture.md`の「本体と任意の外部機能の完成境界」を適用する。GUIの完成・方式・起動・接続状態を本体の一律前提にしない。外部側は本体の公開契約を利用し、具体的な画面や通信方式は外部アダプターと構成処理が所有する。GUIの交換・併用では本体コード・本体設定を変更しない。

#345は任意機能の未完了・品質確認を保持する。本体の#360完了と任意GUI接続の完了を別に記録し、#351の未完了を本体の製造停止へ拡大しない。
