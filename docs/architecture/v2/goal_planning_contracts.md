...
初回の計画確定と、既存計画の置換を区別する。活動失敗などを受けた再計画で、目標を別の内容に変えたり、再計画を通すためだけに目標版を進めたりしない。
...
`GoalPlanningAuthority`は目標ごとの現在計画を保持し、確定の排他区間で置換対象が現在計画と一致することを再検査する。同じ旧計画から競合する再計画が到着しても、確定できるのは1件だけとする。別の計画へ置換済み、未登録、別目標、古い版・能力・方針、不正な候補の場合、旧計画も現在計画への参照も更新しない。

初回確定では既存計画がないことを検査する。既存計画がある場合は、目標全体の版が進んだだけで暗黙に置換せず、置換対象を明示する。成功した新計画には`supersedes_plan_id`を記録する。`snapshot(plan_id)`は旧計画を含む確定内容を返し、`current_plan(goal_id)`は現在採用する1件だけを返す。
...
本節の検証では、同じ目標版の置換、競合候補の排他、旧計画の保持、古い置換対象の拒否、LLM待機中の現在計画変更による拒否、失敗時の状態非更新を確認する。本体の再計画要求からこの入口への接続は後続の結合検証に残る。
