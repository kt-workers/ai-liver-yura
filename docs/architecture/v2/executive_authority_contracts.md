...
6. authoritative requirementsがcandidateの申告から欠落しておらず、申告されたrequired preconditionが開始時snapshotと同じID・subject・predicateのままcurrent stateで判定済みかつ期待値と一致する。開始時snapshotだけで判定せず、同一IDで別条件へ差し替えることも拒否する。
