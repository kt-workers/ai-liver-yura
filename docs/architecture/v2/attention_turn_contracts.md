...
- Executive由来の`AttentionIntentPayload`はoperationを明示的に検証して`AttentionTransition`へ射影する。release系operationはpayloadの対象を新しいState値としては使わず、current Stateをclearする。未知mode、非Attention intent及びraw inputはrejectする。
