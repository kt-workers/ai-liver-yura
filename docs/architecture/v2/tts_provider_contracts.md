...
Bodyはこの公開契約を直接利用する。TTSアダプターも同じ型を生成・利用し、従来の`app.adapters.tts.contracts`からは同一の型を再公開する。型の複製や別型への変換、Body用の意味判断を追加しない。フィールド・列挙値・検証規則と、音声成果・提示報告・時刻の識別子やリビジョンの照合は維持する。
