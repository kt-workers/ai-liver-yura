...
記憶記録と関係は同じ読み取り取引で取得する。同一記録の更新は期待するリビジョンと次のリビジョンを検査し、複数接続から競合しても一方だけが成功する。関係には参照先存在のDB制約を設け、新しい記録・既存記録のリビジョンの変更・関係追加の途中で失敗した場合は全変更を取り消す。保存先の構造のバージョンが未対応なら、自動削除・初期化による復旧を行わない。

保存待ち処理は受理済み要求を容量超過で消失させない。統合可能な待ち要求が存在しない場合も上限を適用する。新しいリビジョンの待ち要求を古いリビジョンで置き換えない。実行中の同期保存へ終了要求が届いた場合は、その処理結果を回収する。保存が確定した結果は DURABLE として実際の要求識別子へ返し、未開始の要求の取消と区別する。保存先自身の期限と合わせて終了を検証する。
...
PostgreSQL の初期保存構造は `yura_v2` 名前空間に作成する。記憶と再起動用状態は、それぞれ保存構造のバージョンを持ち、対応していないバージョンの既存データを削除・初期化しない。状態本体と所有者ごとの保存済みの最大リビジョンは同じ取引で更新する。不採用にした状態の最大リビジョンも保持し、後着した古いリビジョンで巻き戻さない。同じ所有者・種類・リビジョンの重複保存は競合として扱う。

記憶記録と関係には保存JSONの検査値を保持し、読取り時に検査値、記録の識別子・リビジョン、関係の参照先を照合する。状態の写しは既存の `PersistenceSnapshotEnvelope` が計算する検査値とDB上の識別情報を照合する。破損箇所を推測した既定値で修復しない。直接指定した別の正常な記録・所有者は引き続き読み取れる。一括読取りで破損を発見した場合は部分的な結果を完全な結果として返さず、型付き失敗を返す。
...
- 内容: `GoalCommitmentSnapshot.to_dict()` の全フィールド。Goal・Commitmentの状態、条件、参照、由来、時刻、リビジョンを省略しない。
- 外側の所有者状態のリビジョンと取得時刻: 内容の `revision` と `updated_at` に一致させる。
...
- 一部記録の破損、状態のリビジョンの競合、所有者の形式不一致をDB全体の障害へ昇格させない。
...
実DB試験がsubprocessを使用する場合は、必要な`YURA_TEST_POSTGRES_*`接続情報だけを明示的に子processへ継承する。credentialをargvとして渡さず、診断、assertion、stdout、stderr、logへ含めない。親processの全environmentは継承せず、password未指定時は既存CI隔離credential経路を子processでも維持する。試験対象はproduction DBではなく、必ず回収する一時`yura_test_*` databaseとする。
...
起動時は永続化から目標復元候補を取得し、`GoalCommitmentStore(initial)`の公開検査を通して目標所有者を生成する。復元不能は型付き失敗を保持し、保存なしと区別する。起動継続時に古い保存リビジョンを削除・上書きして解決したことにしない。再接続によって運転中の目標を保存状態へ差し戻さない。現在の注意・発話待ち行列・実行途中の活動は復元対象にしない。

確定判断を受けた変更は同期的に`GoalCommitmentStore.apply()`へ渡す。戻り値の確定状態を、所有者のロック外で`persist_goals()`へ投入する。DB完了待ちは目標変更の同期区間へ含めず、状態変更結果と保存確認を分けて呼出し元へ返す。保存が利用不可・受付上限到達・形式制約違反であっても、既に確定した目標変更を巻き戻したり、確定結果を失わせたりしない。型付きの保存失敗を対応する所有者のリビジョンに結び付ける。

保存要求は実行世代と所有者のリビジョンから一意に識別する。接続側は保存結果の無制限な一覧や追加の待ち行列を作らず、既存の保存待ち件数・リビジョンの競合・結果回収契約を使う。変更前に非同期実行環境を検査し、受付手段がないまま目標だけを変更することを防ぐ。

この接続は、最小起動で未接続の認知・判断・目標・発話の全経路を完成とするものではない。本体の構成処理がこの入口へ目標変更を集約し、記憶の保存検索、停止時の回収、別プロセス再起動を併せて段階検証する。

### 復元失敗後の保存保護

復元失敗から暫定的な空の目標所有者で稼働を続けた場合、その実行世代では目標スナップショットの保存を禁止する。DBが再接続できても復元失敗の記録を消さず、所有者のリビジョンが既存の保存リビジョンを超えたことを保存許可へ使わない。現在の目標変更は既存所有者で確定できるが、保存要求は投入せず、そのリビジョンの`FAILED`確認に復元失敗の分類を保持して返す。
...
保存済み目標あり、復元時のDB待機失敗、暫定状態での稼働、DB再接続、既存の保存リビジョンを超えるリビジョンの保存要求、再起動という順序でも、旧保存目標が復元されることを実DBで検証する。

## 本体の記憶保存・検索と振り返り結果の接続

`CoreMemoryPersistenceBinding`は本体からの型付き書込みと検索を、採用済みの`PostgresPersistenceRuntime`へ非同期に渡す。`submit_reflection`は`ReflectionCandidateResult`が保持する受理済み候補をそのまま`MemoryWriteRequest`へ渡し、候補のない拒否結果には保存処理を作らない。期待するリビジョン・対象記憶・関係種別を明示し、既存要求型の検査を通す。関係の示唆や自由文から更新・置換を推測せず、重複・採用・関係の最終処理は既存の記憶所有者へ委譲する。

各受付は`CoreMemoryOperation`を返す。`wait()`の呼出し側が取り消されても、既に投入した保存・検索は回収まで待ち、成功値または型付き保存失敗を操作から取得可能にした後で取消を伝播する。取消を保存失敗とみなして確定済み書込みを否定しない。結果を取り出す入口は同じ操作の`result()`であり、書込みを再投入して確認する方式にしない。

接続側の非同期処理数は構成側が指定する`max_pending`で制限し、受付前に上限と終了状態を確認する。上限到達は既存の`UNAVAILABLE`、終了後は`CLOSED`の型付き保存失敗とする。終了済み操作は接続側の一覧から回収し、結果の保持は呼出し元の操作参照に限定する。追加の無制限な待ち行列や保存履歴を作らない。

`close()`は新規受付を閉じて全操作を回収し、再取消でも途中で終了しない。目標保存と共有するDB資源自体は閉じず、構成側がこの接続を終了してから既存の保存実行基盤を閉じる。検索は正本記憶からの上限付き根拠であり、現在の目標・注意・状態を復元する入口にしない。

実PostgreSQLで、振り返り所有者の受理・拒否、保存後の検索、接続終了と新しい実行基盤での再取得、期待するリビジョンとの競合、取消中に確定した書込みの結果取得を検証する。この接続単体では、通常の認知・振り返り起動と検索根拠の全利用経路まで完成とは扱わない。

## 31. 永続化を利用する本体の起動構成（#359・#334）

`build_persistent_core`は、構成側が明示した未共有の`PostgresPersistenceRuntime`、再接続方針、実行世代、記憶操作の上限、起動設定を受け取る非同期入口とする。呼出し開始から保存実行基盤の終了責任を引き受け、設定不備・起動失敗・取消でも回収する。秘密情報や接続先を新しく探索しない。最終保存と資源終了の猶予が正の設定を必須にし、保存を使わない既定構成の猶予0を暗黙に流用しない。構成側はDB待機・取引・回収に見合う猶予を明示する。

既存の保存実行基盤を起動停止管理へ登録してから起動し、初期接続・復元時の障害も既存の依存状態へ報告する。`CoreGoalPersistenceBinding.restore`による所有者検査後、その同じ接続を本体の目標入口と入力参照へ渡す。別の空の目標所有者を並立させない。初回接続の失敗は依存状態に報告し、復元失敗は目標接続に保持する。再接続はDBの利用可能性だけを回復し、目標の復元や保存抑止解除を繰り返さない。記憶操作は同じDBの`CoreMemoryPersistenceBinding`へ渡す。

本体停止は、認知処理の停止・回収、記憶操作の受付終了・回収、依存機能の終了の順で行う。目標保存の回収とDB終了は既存の保存実行基盤に委譲する。記憶回収は最終保存猶予で監視し、超過は回収後に型付きの停止失敗として返す。停止要求の取消・再取消でも共有する終了タスクを回収してから取消を返す。前段の失敗でも後段の終了を実行する。

この入口の採用は、接続先・秘密情報の配備設定、通常認知からの全操作、実サービス品質の完成を意味しない。既存の最小起動入口は保存なしの構成として保持する。実DB上の保存済み目標からの起動、同じ参照入口での更新観測、復元失敗後の保護、記憶操作と終了、起動失敗・取消の回収を検証する。

### 31.1 永続化境界の結合受入れと採用証拠

#359の結合受入れは、採用済みの保存先（PR #589）、目標接続・復元失敗後の保存保護（PR #594）、記憶接続（PR #598）、本体起動停止（PR #602）を使用する。次の試験は`tests/infrastructure/postgresql/`で実PostgreSQLを使用する。

| 受入条件 | 確認する公開経路と試験 |
| --- | --- |
| 本体の目標・記憶の保存と復元 | `test_persistent_boot.py`の本体起動から目標変更・記憶操作を行い、同じ入力参照で復元した目標と更新を観測する試験 |
| 別プロセス終了後の耐久性 | 同ファイルの子プロセスが`build_persistent_core`から保存確認まで進み、終了処理を省略して終了する試験。別プロセスの同じ本体起動入口で目標・記憶を復元する。保存確認前の成功は仮定しない |
| DB不在中の本体継続 | 同ファイルのDB不在起動で、型付きの保存失敗を保ち、認知処理の受付と結果取得を継続する試験 |
| 実DB障害後の自動再接続 | 同ファイルの隔離DBが新規接続を拒否する状態から、登録済みの再接続処理で記憶の書込み・検索を回復する試験。目標所有者と復元失敗時の保存抑止を置き換えない |
| 復元失敗後の既存保存状態の保護 | `test_core_goal_binding.py`と本体起動試験の、実DBのロックによる復元失敗・再接続・暫定目標の更新・再起動を通す試験 |
| 取消・最終保存猶予・資源回収 | 本体起動試験の起動取消、停止再取消、最終保存猶予超過後の型付き停止失敗と回収。`test_core_memory_binding.py`と`test_runtime.py`の実DB処理中の取消、確定結果の保持、終了時のタスク回収 |
| 保存の競合・破損・形式不一致 | `test_memory.py`と`test_snapshots.py`の取引・期待するリビジョン・整合性・形式拒否と、既存状態を削除しない試験 |
