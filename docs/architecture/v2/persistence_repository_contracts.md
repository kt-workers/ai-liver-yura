...
`CoreMemoryPersistenceBinding`は本体からの型付き書込みと検索を、採用済みの`PostgresPersistenceRuntime`へ非同期に渡す。`submit_reflection`は`ReflectionCandidateResult`が保持する受理済み候補をそのまま`MemoryWriteRequest`へ渡し、候補のない拒否結果には保存処理を作らない。明示する期待版・対象記憶・関係種別は既存要求型の検査を通す。関係の示唆や自由文から更新・置換を推測せず、重複・採用・関係の最終処理は既存の記憶所有者へ委譲する。
...
実PostgreSQLで、振り返り所有者の受理・拒否、保存後の検索、接続終了と新しい実行基盤での再取得、期待版競合、取消中に確定した書込みの結果取得を検証する。この接続単体では、通常の認知・振り返り起動と検索根拠の全利用経路まで完成とは扱わない。
