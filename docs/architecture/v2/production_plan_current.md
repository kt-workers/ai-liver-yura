...
状態: #550 改訂7 / 実装優先・親Issue単位Gate（2026-09-08）
...
照合した本流は `rebuild/v2-foundation@27bf6181cd0007e3c69768f361274dabb2249f50`。この値を次回の現在値として固定しない。
...
今回は#550を一時再開し、`AGENTS.md`と本書の2文書だけに製造方式変更を反映する。製品コード・設計正本の意味・#632/#630の製品実装は変更しない。管理PRは1本を通常mergeし、#550をcompletedへ戻して読み戻す。この管理更新の独立レビューは新設しない。検証は変更ファイル・Markdown参照・diff-check・旧新規則の矛盾・#207/#450/#550とのAuthority整合だけとし、製品のfull pytest・Ruff・Mypy・compileallは実行しない。自動必須CIは1回取得する。#450へのCheckpointを1件記録して停止し、#632実装は次の作業とする。Mission終了ではない。
...
#325・#356は未完の子を持つ上位Parentとして管理を再開した。既存の実装完了と採用証拠を取り消したものではない。#329・#333・#361は管理上の整理だけでCloseせず、所有者固有の受入条件の照合を残す。不具合は同じIssueのTest/Fixで扱い、冒頭節の独立責務・Owner・完成・延期の条件を満たす場合だけ分離する。

## 3. 現在の依存関係と工程

次表は改訂6の再構成時の値を基に、#630/#632の追加と最新状態を反映したものである。工程は依存を満たした後の優先順を示し、開始日の代用ではない。Parent全体の完了を子Workの前提にせず、必要な成果を直接依存にする。レビュー待ちはReviewとし、未完成をDoneに読み替えない。

| Issue | 責務 | 区分 | 直接依存 | Status | 工程 |
| --- | --- | --- | --- | --- | --- |
| #632 | 所有者更新と最終確定を世代Fenceで直列化する | Work | #321, #322 | Ready | 未設定 |
| #630 | 意図ごとの必須要件を正規の根拠から導出する | Work | #328, #632 | Blocked | 未設定 |
| #610 | 判断の必須要件・前提条件・計画根拠の本番供給を成立させる | Work | #323, #328, #343, #366, #630 | Blocked | 1 |
...
| 区分 | 件数 | 対象 |
| --- | --- | --- |
| 本体Work | 10 | #329・#333・#361・#610・#616・#617・#618・#619・#630・#632 |
...
| 今回の管理作業 | 1 | #550。本改訂を通常mergeしてcompletedへ戻す。本体集計の対象外 |
| 継続Mission管理 | 1 | #450。本体タスクとは別に数える |
| 本体完成後の性能管理 | 1 | #586。Backlog・未着手・今回対象外 |

本体と後回し外部の確定した実タスクは計29件。必要性再評価8件は完了扱いにしない。#566の[後発記録](https://github.com/kt-workers/ai-liver-yura/issues/566#issuecomment-5558274318)は、七つの失敗返却API変更を一律必須とした判断を取り下げている。現在の正本に対して必要性が立証されるまで、本体の必須依存と実タスク数へ機械的に加えない。既存提案・作業系統・Project状態を保持し、必要な欠陥が実際に確認された範囲だけ再判定する。
...
次は **#632（OPEN / Ready）のimplementation** とする。設計PR #633は指定基底`27bf6181cd0007e3c69768f361274dabb2249f50`へ採用済み。#632を実装・Test/Fix・最終Gate・採用まで完了した後、#630（OPEN / Blocked）を既存系統で再開する。#610は#630の完了を待つ。既存の#630未コミット試作にはこの管理作業で触れない。

参照する正本:

- [並行動作設計](concurrency_architecture.md)
- [Foundation契約](foundation_contracts.md)
...
今回の管理変更は本体残23件を増減しない。改訂6の21件に既に登録済みの#630/#632を反映した表示更新であり、新しい製品Issueを作るものではない。#632実装開始時にfresh Resume Gateを行い、同一Issue内の工程遷移では繰り返さない。今回は管理変更の採用・読み戻し・Checkpointで停止する。
