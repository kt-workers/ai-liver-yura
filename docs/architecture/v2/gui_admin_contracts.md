...
Stage Aはブラウザ画面とし、サーバは**asyncioとh11の公開APIによるHTTP/JSONスナップショット通信**、画面は**素のHTML / CSS / JavaScript**を採用する。現在の本番実行基盤であるasyncioに接続し、同じ実行基盤内でサーバの起動停止を所有するための選択である。別のASGIサーバプロセスを要求しない。将来の通信拡張の可能性は、Stage Aでその機能を実装する許可を意味しない。
...
- Stage Aの現在の実装方式の採用は、PyQt / Qt等による将来のGUI交換・併用を禁止しない。今回は方式の移行や別GUIの製造を行わない。本体の公開契約を利用し、交換・併用のための本体コード・本体設定の変更を要求しない。

2026-09-06の依存方針監査ではPipfileとPipfile.lockを変更対象外とした。2026-09-22の通信方式変更では、既存lockのh11 0.16.0をGUIの直接依存として明示し、他パッケージの解決結果を変更しない。パッケージの存在だけで本体の依存違反とは判断しない。GUIパッケージの除去や、それらを導入しない環境での試験を本体の完成条件にしない。
...
各接続は1応答に限定する。成功・失敗を問わず応答完了後に接続を閉じ、アイドル状態のHTTP持続接続を保持しない。asyncioの通信処理で応答後の接続を明示的に閉じ、既定の持続接続待機時間へ依存しない。Stage A第1版では持続接続無効を正本とし、将来の有効化には通信方針の版更新を必要とする。
...
- GUI方式の交換・併用を本体の変更なしで可能にする公開境界を確認する。通信方式の直接依存宣言以外にパッケージ構成を変更せず、パッケージ除去を検証前提にしない。

### 19.10.1 HTTP解析と接続境界の追加必須試験

ヘッダー件数の正本は数値契約第4.1節の`max_header_count = 64`とする。単一ヘッダーの長さや総バイト数とは別の上限であり、通信受付がh11へ渡す前に行の長さとヘッダー件数を検査する。CRLFを除く要求行と各ヘッダー行に上限を適用し、分割受信でも一括受信でも同じ条件を守る。
...
12. 通信ライブラリの持続接続・ヘッダー上限の既定値へ暗黙に依存しないことを確認する。
...
### 19.12 任意GUIの起動停止の実装境界

`GuiAdminSubsystem`は同一の`MinimumBrainProductionConfig`、GUI設定、時計、明示的な通信factoryを受け取る。通信factoryは構築時に待受を開始せず、`GuiAdminTransport.start()` / `stop()`が通信資源を所有する。具体的な通信方式を暗黙選択したり、未接続の通信先を成功として返したりしない。

並行するstart/stopはそれぞれ一つの作業を共有する。起動失敗でも部分的な通信資源の回収を試み、生例外を公開しない。停止開始後は同じSubsystemインスタンスを再起動せず、再起動が必要なら構成所有者が新しいインスタンスを作る。Coreの状態を停止・変更しない。

停止は通信方針の上限内で待機する。非協調の作業を回収できない場合は`GuiAdminStopResult.completed = false`、残存作業数、`GUI_SHUTDOWN_TIMED_OUT`を返し、正常停止を偽装しない。停止処理の失敗は`GUI_SHUTDOWN_FAILED`で保持する。後から作業が収束した場合も再度stopで資源回収を確認できる。

具体的なfactoryには`GuiAdminHttpTransport`を注入する。2026-09-22のユーザー指示に従いaiohttpを使用せず、既存案のasyncio＋h11を採用する。h11へ渡す前に受信行を有界に検査し、解析失敗には入力・例外詳細を含まない固定400・本文0バイトを返す。要求本文を読み込まず、1接続1応答で閉じる。受付数は解析中・送信中を含め、上限超過は待機キューへ入れず503で閉じる。メソッド確定前の上限拒否は本文を省略する。通常応答は503を含めwrite後のcloseで送信バッファのflushを待ち、直後にabortしない。応答開始時に要求期限タイマーを解除し、connection_lostで枠を回収する。上限拒否の送信中接続も停止時の回収対象として保持するが、要求受付枠や処理待ちキューへは追加しない。shutdownでは受付済み・拒否応答送信中の接続をともに強制回収する。

自動試験の結果をHTTP待受・ブラウザの実動作や本番構成への登録の証拠へ昇格しない。実動作の確認はHumanが所有する。
