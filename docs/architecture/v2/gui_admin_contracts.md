...
PipfileとPipfile.lockのパッケージ構成は今回の監査・修正対象外とし、変更しない。パッケージの存在だけで本体の依存違反とは判断しない。GUIパッケージの除去や、それらを導入しない環境での試験を本体の完成条件にしない。
...
- GUI方式の交換・併用を本体の変更なしで可能にする公開境界を確認する。今回の修正でPipfile／Pipfile.lockを変更せず、パッケージ除去を検証前提にしない。
...
通信方式未選定の段階ではfactory契約と模擬通信先の自動試験だけを実施する。HTTP待受・ブラウザの実動作や本番構成への登録の証拠へ昇格しない。実動作の確認はHumanが所有する。
