...
具体的なfactoryには`GuiAdminHttpTransport`を注入する。2026-09-22のユーザー指示に従いaiohttpを使用せず、既存案のasyncio＋h11を採用する。h11へ渡す前に受信行を有界に検査し、解析失敗には入力・例外詳細を含まない固定400・本文0バイトを返す。要求本文を読み込まず、1接続1応答で閉じる。受付数は解析中・送信中を含め、上限超過は待機キューへ入れず503で閉じる。メソッド確定前の上限拒否は本文を省略する。
