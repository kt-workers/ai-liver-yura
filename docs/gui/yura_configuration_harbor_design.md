...
スマートフォンの縦長ページでは、背景をビューポート固定の一枚絵として扱わない。

- `body`自身に海中グラデーションを持たせ、背景要素が描画されない場合も黒い空白を出さない
- 背景レイヤーは`position: absolute`でページ全体の高さへ追従させる
- 海面と光線はページ上部、海底・海草・岩はページ下部に配置する
- 中間部分は水中グラデーションと光模様を連続させる
- iOS Safariで不安定になりやすい長大な`position: fixed`背景へ依存しない
- 860px以下では装飾の高さと位置をモバイル向けに調整する
- 560px以下では海底装飾を縮小し、フォームの可読性を優先する
