...
owner fileは内部実装で管理する。利用者向け画面にはファイル名やmanifestパスを常設表示しない。
...
manifest、revision、owner file一覧などの技術情報はAPI内部で保持するが、通常画面には表示しない。
...
- 上部Summaryカードは設けず、作業領域へ高さを割り当てる
...
設定ファイル名は実装上必要だが、利用者が設定を操作するうえでは不要な情報として扱う。

- `config/index.yaml`などのmanifestパスを表示しない
- カテゴリ一覧に`runtime.yaml`などのowner file名を表示しない
- 編集見出しに現在のowner file名を表示しない
- 各設定項目に「保存先: xxx.yaml」を表示しない
- 画面上ではカテゴリ名、設定名、入力値、検証状態、反映方法を優先する
- owner file名はサーバー内部、保存処理、監査情報でのみ保持する
