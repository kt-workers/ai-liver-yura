...
本資料は、本番設定を`config/index.yaml`から読み込む現行構成へ、環境別の差分設定を安全に追加するための最終方針を定める。

対象は開発環境、テスト環境、ローカル実行環境、将来の本番環境である。

## 2. 現行保証

現行loaderは次を保証している。

- 各トップレベルキーは単一のowner fileだけが所有する
- owner file間のdeep mergeを行わない
- nested importsを禁止する
- 未知キーと重複キーを拒否する
- `ConfigError.source_file`は設定を記述したowner fileを示す
- legacy単一設定とmanifest設定の等価性を検証する

環境別overrideは、これらを崩してはならない。
...
`config/index.yaml`へ任意の`environments`を追加する。
...
`environments`は環境名とoverride fileの対応だけを持つ。

同じ環境名の重複、空path、未知のmanifestキーは拒否する。

### 3.3 override file形式

暗黙deep mergeは採用しない。overrideは明示的なpath操作として記述する。
...
各要素は`path`と`value`だけを持つ。

未知フィールド、重複path、空pathは拒否する。

## 4. v1で許可する変更

v1では既存設定に存在するleaf値の置換だけを許可する。
...
override file自体の構造エラーは常にoverride fileをsourceとする。

## 7. secretの扱い

環境別overrideはsecret管理機能ではない。

API keyやpasswordは既存どおり環境変数名を設定へ記述し、実値はOS、CI、クラウドのsecret機能から取得する。

override fileへ実secret値を保存しない。

## 8. legacy設定との関係

v1のenvironment overrideはmanifest entryにだけ対応する。

`config/config.yaml`などlegacy単一設定を明示指定した場合、`AI_LIVER_CONFIG_ENV`が設定されていれば明確な`ConfigError`にする。

暗黙的にoverrideを無視しない。

これによりlegacy互換経路の意味を変えず、移行漏れを検出できる。
...
これらは設定結果とsourceの予測可能性を下げるためである。

## 13. 完了条件

次を満たした時点でenvironment override実装を完了とする。
