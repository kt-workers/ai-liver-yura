...
Memory PluginはCoreドメイン型を直接受け取らず、Sharedのversioned `AgentMemorySnapshot`
と`AgentMemoryStore`契約だけを扱う。保存失敗時は`memory.agent_state`を解除し、Coreは
上限付きインメモリ状態でEvent処理とActivity管理を継続する。
Relationship Memory PluginもSharedの型付き`SnapshotStore`だけを扱い、関係性の更新・
値域検証はCore Domain、保存方式はAdapterに残す。

契約の正本は`app/shared/contracts/plugins/`に置く。常駐Runtime向けの同期Capability契約は
`runtime` profile、管理・Streaming orchestration向けの非同期Registration契約は
`registration` profileとして明示し、Pluginから`app/core`をimportしない。
汎用Registry / Dispatcher / Event Brokerは`app/shared/plugin_host`と
`app/shared/observability`に置く。旧`app/core/...`パスは移行期間の互換re-exportだけとする。

Voice Output Pluginは`output.speech`を公開し、SpeechSynthesizer / AudioPlayer Portの
組が利用可能な場合だけCapabilityを登録する。合成または再生失敗時はPlugin Contextの
Capability Reporterで即座に解除し、Coreはテキスト出力と失敗Resultの記録を継続する。
VoiceIntent、SpeechSynthesizer、AudioPlayerの正本はShared Contractsとし、Pluginは
CharacterResponseやCore側Portをimportしない。旧Domain/Portsパスは互換re-exportに限定する。
...
明示的な再接続要求では、対象Pluginだけを停止・再初期化し、宣言済みかつ現在利用可能な
Capabilityを再検出してRegistryへ登録する。明示的に無効化または停止済みのPluginは
自動復旧対象にせず、復旧失敗はPlugin単位で隔離する。
...
Registration profileのActivity ProviderはCoreの`Activity`を直接生成せず、
`PluginActivitySpec`を返す。Specはactivity_type、goal、priority、context、
interruptible、source_event_id、trace_idだけを持ち、Composition RootがCore Activityへ変換する。
...
`state`は`PluginActivityState`であり、CoreがPlugin固有Session型を参照せずに
継続状態を同期するため、`session_id`、`status`、`expected_input`、
`end_condition`を持つ。statusは`waiting_input`、`suspended`、`completed`、
`canceled`のいずれかとする。
...
OngoingActivityは共通キー`plugin_session_id`でPlugin Sessionを参照する。
ゲーム盤面や使用済み単語などのPlugin固有状態は保持しない。
...
- 再試行には上限を設ける
- 再接続はPlugin単位で行い、復旧後にCapabilityを再検出する
...
LLM Providerはdefault、Situation Evaluator、Character、Response Validatorを役割別の
Capabilityとして登録する。一つのProviderが失敗しても他の役割のCapabilityは維持し、
失敗した役割はCore側の決定論的または安全な応答フォールバックへ戻す。
Provider PluginはSharedの`ResponseGenerationGateway`を境界とし、Core Activity型や
Core側ResponseGenerator Portへ依存しない。

Plugin内部LLM処理はSharedの`PluginLlmRequest`、一時的な生成作業は
`PluginActivityWorkItem`としてCoreへ渡す。Core Activityへの変換はcomposition rootの
...
独立Subsystemとの接続は、同一プロセス内Pluginとは別のIntegration境界として扱う。
